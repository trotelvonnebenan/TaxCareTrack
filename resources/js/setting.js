...
function renderSettings() {
    loadSettings().then(function (setting) {
        ...
        if (typeof (setting.idle_detection) !== "undefined") {
            if (setting.idle_detection == 1) $('#idle_detection').prop('checked', true);
            else $('#idle_detection').prop('checked', false);
        }
        if (typeof (setting.idle_timeout) !== "undefined") {
            $('#idle_timeout').val(setting.idle_timeout);
        } else {
            $('#idle_timeout').val(5);
        }
